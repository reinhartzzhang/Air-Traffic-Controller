...
# The following while loop is what i have calculated. You can run it to see
# what is happening and then discard it completely and do your own calculations
# to correct it. The while loop should be responsible for tangentially drawing
# a circular path as explained.

##    l_p=202
##    while(delay>0):
##        alpha=l_p*np.cos(math.radians(theta)+25/150.0)+25*np.cos(math.radians(theta_inc))
##        beta=-l_p*np.sin(math.radians(theta)+25/200.0)-25*np.sin(math.radians(theta_inc))
##        theta_inc=theta_inc+15
##        delay=delay-1
##        y=str(int((alpha*alpha+beta*beta)**0.5)).zfill(4)
##        x=str(int(math.degrees(math.atan2(beta,alpha)))).zfill(4)
##        clisocket.send(x)
##        clisocket.send(y)
